 ...
 * 
 * cache.put(1,1);cache.put(2,2);cache.get(1); // returns 1 cache.put(3,3); // evicts key 2
 * cache.get(2); // returns -1 (not found) cache.get(3); // returns 3. cache.put(4,4); // evicts key
 * 1. cache.get(1); // returns -1 (not found) cache.get(3); // returns 3 cache.get(4); // returns 4
 */
...
public class LFUCache {
    ...
    /**
     * Two HashMaps are used, one to store <key, value> pair, another store the <key, node>. I use
     * double linked list to keep the frequent of each key. In each double linked list node, keys
     * with the same count are saved using java built in LinkedHashSet. This can keep the order.
     * Every time, one key is referenced, first find the current node corresponding to the key, If
     * the following node exist and the frequent is larger by one, add key to the keys of the
