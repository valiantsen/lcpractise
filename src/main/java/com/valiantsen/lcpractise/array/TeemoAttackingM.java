 ...
 * 
 * @ClassName: TeemoAttackingM
 * @Description: TODO(这里用一句话描述这个类的作用)
 * @author zhangyasen
 * @date 2017年5月7日
 */
public class TeemoAttackingM {
    ...
    public int findPoisonedDuration(int[] timeSeries, int duration) {
        ...
        if (timeSeries.length > 0) {
            for (int i = 0; i < timeSeries.length; i++) {
                if (timeSeries[i] < endHere) {
                    totalDura += (timeSeries[i] + duration - endHere) > 0 ? (timeSeries[i] + duration - endHere) : 0;
                    endHere = Math.max(timeSeries[i] + duration, endHere);
                } else {
                    totalDura += duration;
                    endHere = timeSeries[i] + duration;
                }
            }
        }
        return totalDura;
